...
offsets = -1*[0.01 10];%[ 1 -20 -10 -7 7 10 20 ];
...
for kk = 1:Nplot
    ...
    switch sensors{kk}
        case 'asq'
            ...
            TF{kk} = [f,calTF(pickleTF(result(kk),'AM','AS_Q',olcl),calASQ_DARMm)];
        case 'omc'
            calOMC_DARMm = 1./((pickleTF(result(kk),'EX','OMC_PD',olcl)-pickleTF(result(kk),'EY','OMC_PD',olcl)));
            TF{kk} = [f,calTF(pickleTF(result(kk),'AM','OMC_PD',olcl),calOMC_DARMm)];
    end
end
